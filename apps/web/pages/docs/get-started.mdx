...
In this walkthrough we are going to get started to build a search experience with Next.js. We will
...
```sh
  yarn add @instantsearch-elasticsearch-adapter/client @instantsearch-elasticsearch-adapter/api react-instantsearch-dom
```
...
import createClient from "@instantsearch-elasticsearch-adapter/client";
...
Below we are using the `Highlight` component from `react-instantsearch-dom` to highlight the search term in the title and actors fields. We need to pass the `attribute` prop to the `highlight_attributes` config to tell which fields to bring highlight options for.
...
`imdbrating` field is a `float` type field.
